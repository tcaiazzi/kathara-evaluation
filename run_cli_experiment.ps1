...
foreach ($dir in Get-ChildItem -Path "$scenarios_path" -Directory) {
    ...
    for ($i = 0; $i -le 10; $i++) {
        ...
        kathara linfo -d .\network-scenarios\kathara-lab_bgp-ospf-rip\ | Select -skip 1 | Select-String -Pattern '[0-9]+[.][0-9]+ ' |  ForEach-Object { $_.Matches.Value.Split('|')[0] } | Out-File -FilePath "$result_path\$os_name\mem\$lab-log-$i.txt"
